...
/**
 * Level 1 Component - MCQ Questions
 * 
 * Gameplay:
...
const Level1 = () => {
    ...
    const [currentQuestionIndex, setCurrentQuestionIndex] = useState(() => {
        try {
            const saved = sessionStorage.getItem('level1_progress');
            if (saved) {
                const progress = JSON.parse(saved);
                return progress.currentQuestionIndex || 0;
            }
        } catch (e) {
            console.error('Error loading Level 1 progress:', e);
        }
        return 0;
    });
    ...
    const [results, setResults] = useState(() => {
        try {
            ...
            console.error('Error loading Level 1 results:', e);
        }
        return [];
    });
    ...
    useEffect(() => {
        const handleForceSubmit = () => {
            console.log('[LEVEL 1] Anti-cheat force submit triggered');
    ...
    const handleQuestionComplete = (result) => {
        ...
        if (currentQuestionIndex < level1Questions.length - 1) {
            ...
            try {
                sessionStorage.setItem('level1_progress', JSON.stringify({
                    currentQuestionIndex: nextIndex,
                    results: newResults
                }));
            } catch (e) {
                console.error('Error saving Level 1 progress:', e);
            }
            ...
            try {
                sessionStorage.removeItem('level1_progress');
            } catch (e) {
                console.error('Error clearing Level 1 progress:', e);
            }
    ...
    const processLevelCompletion = async (finalResults) => {
        ...
        try {
            ...
            recordLevelAttempt(1, cleared, totalScore);
            ...
            <div className="level-container">
                ...
                <div className="level-content">
                    <div className="level-header">
                        <h1 className="level-title">LEVEL 1: MCQs</h1>
                    </div>
